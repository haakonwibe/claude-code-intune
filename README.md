...
A small developer toolkit packaged as Win32 apps for the Intune
Company Portal. The main app is Claude Code. Claude Desktop,
VS Code, PowerShell 7, and Git for Windows come along as well.
...
| Claude Desktop  | Anthropic CDN MSIX (`claude.ai/api/desktop/...`)  | System  |
...
All five apps are separate installs in Intune. None of them is set
up as a Win32 dependency of any other. Claude Code works on its own.
Users can install the other four from Company Portal when they want
them.
...
2. `.\source\Update-Installers.ps1` - downloads the latest Git for
   Windows, VS Code (System x64), PowerShell 7, and Claude Desktop
   installers into the per-app `package/` folders. Each download is
   signature-checked.
...
- Win32 install/uninstall scripts re-launch themselves as 64-bit
  via `sysnative` when spawned by IME's 32-bit process.
...
- Install and uninstall scripts isolate cleanup-step errors from
  the main exit code; a failed cleanup never hides install/uninstall
  success or failure.
- Exit code contract: 0 = success, 1 = failure, 3010 = success with
  ...
  behavior** setting. Only Claude Desktop's `-EnableCowork` path
  currently emits 3010, when DISM's VMP enable queues a reboot.

## Out of scope

- ARM64.
- Devices that are not joined to Entra ID.
- Sign-in automation. Users sign in to Claude Code and Claude
  Desktop themselves on first run.

## License

MIT - see [LICENSE](LICENSE).

*Not affiliated with Anthropic. "Claude", "Claude Code", and "Claude
Desktop" are trademarks of Anthropic, PBC, used here only to identify
the upstream products this kit deploys.*
