...
try {
    ...
    # Write the detection marker. By this point the binary exists
    # and the version check passed, so the install has succeeded. A
    # marker-write failure is a detection concern, not an install
    # concern, so it stays non-fatal.
    try {
