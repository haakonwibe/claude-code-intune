...
.DESCRIPTION
    ...
    Git is already absent - uninstall reports success so re-runs and
    clean machines both report green.

    Logs in CMTrace format to: %ProgramData%\Hawkweave\ClaudeCodeIntune\Logs\GitForWindows-uninstall.log

.NOTES
    Companion app - independent of Claude Code. Removing Git via Company
    Portal has no effect on Claude Code itself.
#>
