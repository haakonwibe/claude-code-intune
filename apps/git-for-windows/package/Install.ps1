...
.DESCRIPTION
    ...
    apps\git-for-windows\package\ by source\Update-Installers.ps1 at
    packaging time and bundled into the .intunewin alongside this script.
    winget is unsupported in SYSTEM context per Microsoft docs; see
    ARCHITECTURE.md ("Direct installer Win32 packages") for the pivot.

    Logs in CMTrace format to: %ProgramData%\Hawkweave\ClaudeCodeIntune\Logs\GitForWindows-install.log

.NOTES
    Companion app - independent assignment to the Developers group, not
    a Win32 dependency of Claude Code. Users install on demand via
    Company Portal.
#>
...
try {
    # Locate the bundled installer next to this script. The .intunewin
    # extracts every file from package/ into the same temp dir at install
    # time, so $PSScriptRoot is the right anchor.
    $candidates = Get-ChildItem -Path $PSScriptRoot -Filter 'Git-*-64-bit.exe' `
                                -File -ErrorAction SilentlyContinue |
                      Sort-Object Name -Descending
    if ($candidates.Count -eq 0) {
        throw "Bundled Git for Windows installer (Git-*-64-bit.exe) not found in $PSScriptRoot. Run .\source\Update-Installers.ps1 and rebuild the .intunewin."
    }
    ...
    #   /NOCANCEL            don't render a cancel button (no UI anyway,
    #                        but blocks Ctrl-C from aborting)
    #   /SP-                 skip the legacy "this will install..." prompt
    #   /CLOSEAPPLICATIONS   close apps holding Git files in use
    #   /RESTARTAPPLICATIONS restart them after install
    #   /COMPONENTS=...      pin the optional components so devices have a
    #                        uniform Git layout. Quotes around the value
    #                        are unnecessary (no spaces) and avoiding them
    #                        sidesteps PowerShell argument-quoting quirks.
    $installArgs = @(
        '/VERYSILENT',
        '/NORESTART',
        '/NOCANCEL',
    ...
    # Inno Setup: 0 success, 3010 success-with-pending-reboot. Anything
